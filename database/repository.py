...
def get_statistics():
    ...
    cursor.execute("SELECT crime_type, COUNT(*) as count FROM firs GROUP BY crime_type ORDER BY count DESC LIMIT 1")
    top_crime_row = cursor.fetchone()
    top_crime = top_crime_row['crime_type'] if top_crime_row else 'N/A'
    
    cursor.execute("SELECT location, COUNT(*) as count FROM firs GROUP BY location ORDER BY count DESC LIMIT 1")
