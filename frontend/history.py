...
import json
import os
from database.repository import get_all_complaints, search_complaints, get_complaint_by_id, delete_complaint, get_statistics
...
def render_history_page():
    ...
    crime_types = ["All"] + sorted(list(set([c['crime_type'] for c in all_complaints if c['crime_type']])))
    ...
    search_query = st.text_input("🔍 Search across complaints (ID, Name, Location, Summary, Vehicles)...", "")
    ...
    with colB:
        ...
        st.download_button(label="📥 Export All (CSV)", data=csv, file_name="all_complaints.csv", mime="text/csv")
    ...
    if total_pages > 1:
        st.session_state.page_number = st.number_input("Page", min_value=1, max_value=total_pages, value=st.session_state.page_number)
...
def render_complaint_details(fir_id):
    ...
    with st.expander("View JSON Payload", expanded=False):
        ...
        st.download_button("📥 Download JSON", data=json_str, file_name=f"complaint_{details.get('id')}.json", mime="application/json")
